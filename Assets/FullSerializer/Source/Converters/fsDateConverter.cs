    ...
    public class fsDateConverter : fsConverter {
        ...
        public override fsResult TrySerialize(object instance, out fsData serialized, Type storageType) {
            if (instance is DateTime) {
                ...
                serialized = new fsData(dateTime.ToString(DateTimeFormatString, DateTimeFormatInfo.InvariantInfo));
                return fsResult.Success;
            }

            if (instance is DateTimeOffset) {
                var dateTimeOffset = (DateTimeOffset)instance;
                serialized = new fsData(dateTimeOffset.ToString(DateTimeOffsetFormatString, DateTimeFormatInfo.InvariantInfo));
                return fsResult.Success;
            }
        ...
        public override fsResult TryDeserialize(fsData data, ref object instance, Type storageType) {
            ...
            if (storageType == typeof(DateTime)) {
                DateTime result;
                if (DateTime.TryParse(data.AsString, DateTimeFormatInfo.InvariantInfo, DateTimeStyles.RoundtripKind, out result)) {
                    instance = result;
                    return fsResult.Success;
                }
                ...
                if (fsGlobalConfig.AllowInternalExceptions) {
                    try {
                        instance = Convert.ToDateTime(data.AsString, DateTimeFormatInfo.InvariantInfo);
                        return fsResult.Success;
                    }
                    catch (Exception e) {
            ...
            if (storageType == typeof(DateTimeOffset)) {
                ...
                if (DateTimeOffset.TryParse(data.AsString, DateTimeFormatInfo.InvariantInfo, DateTimeStyles.RoundtripKind, out result)) {
                    instance = result;
                    return fsResult.Success;
                }
