...
namespace PolyNav{
	...
	public class PolyNav2D : MonoBehaviour {
		...
		private HashSet<PolyNavObstacle> transientNavObstacles = new HashSet<PolyNavObstacle>();
		...
		public static readonly string POLYNAV_GO_NAME = "@PolyNav2D";
		...
		void MonitorObstacle(PolyNavObstacle obstacle, bool active){
			if ( obstaclesMask == (obstaclesMask | 1 << obstacle.gameObject.layer) ){
				if (active && obstacle.gameObject.transform.parent.gameObject.name.Equals(POLYNAV_GO_NAME)) {
		...
		public void AddTransientObstacle(PolyNavObstacle navObstacle) {
			AddObstacle(navObstacle);
			transientNavObstacles.Add(navObstacle);
		}
		...
		public void RemoveTransientObstacle(PolyNavObstacle navObstacle) {
			RemoveObstacle(navObstacle);
			transientNavObstacles.Remove(navObstacle);
		}
		...
		public void FindPath(Vector2 start, Vector2 end, Action<Vector2[]> callback, PolyNavAgent agent, bool disableCollision = false){

			if (disableCollision || CheckLOS(start, end)){
				callback( new Vector2[]{start, end} );
				return;
			}
			
			Func<Boolean> onFail = delegate {
				if (Mathf.Approximately(0f, agent.currentSpeed)) {
					navObstacles = navObstacles.Except(transientNavObstacles).ToList();				
					GenerateMap(false);				
			...
			pathRequests.Enqueue( new PathRequest(start, end, callback, onFail) );
			TryNextFindPath();
		}
		...
		void RequestDone(Vector2[] path) {					
			...
			if (path == null && currentRequest.onFail != null) {
				if (!currentRequest.onFail()) { //If the fail handler did not do anything
			...
			if (generateMaster){
				...
				if (masterCollider is PolygonCollider2D){
					...
					for (int i = 0; i < polyCollider.pathCount; ++i){
						...
						var inflated = InflatePolygon(transformed, Mathf.Max(0.0f, radiusOffset) );
	...
	*/
		...
		struct PathRequest{
			...
			public Func<Boolean> onFail;

			public PathRequest(Vector2 start, Vector2 end, Action<Vector2[]> callback, Func<Boolean> onFail = null){
				this.start = start;
				this.end = end;
		...
		[MenuItem("GameObject/Other/PolyNav Map")]
		[MenuItem("Tools/ParadoxNotion/PolyNav/Create PolyNav Map")]
		public static void Create(){
			var newNav = new GameObject(POLYNAV_GO_NAME).AddComponent<PolyNav2D>();
			Selection.activeObject = newNav;
		}

		[MenuItem("GameObject/Other/PolyNav Obstacle")]
		[MenuItem("Tools/ParadoxNotion/PolyNav/Create PolyNav Obstacle")]
		public static void CreatePolyNavObstacle(){
			var obs = new GameObject("PolyNavObstacle").AddComponent<PolyNavObstacle>();
			Selection.activeObject = obs;
		}
